...
class MainWindow(QMainWindow):
    ...
    def try_init_nvml(self):
        try:
            try: import pynvml
                ...
                vlib = os.path.join(VENV_DIR, "lib", "python3.11", "site-packages")
    ...
    def setup_train_tab(self):
        ...
        self.btn_setup = QPushButton("PREPARAR AMBIENTE (Python 3.11)"); self.btn_setup.clicked.connect(self.start_setup)
        ...
        try:
            for card in ["card0", "card1"]:
                ...
                if os.path.exists(f"{base}/gpu_busy_percent"):
                    with open(f"{base}/gpu_busy_percent", "r") as f: load = f"{f.read().strip()}%"
                    temp = "0°C"
                    hwmon = f"{base}/hwmon"
                    if os.path.exists(hwmon):
                        for hdir in os.listdir(hwmon):
                            tf = f"{hwmon}/{hdir}/temp1_input"
                            if os.path.exists(tf):
                                with open(tf, "r") as f: temp = f"{int(f.read().strip()) // 1000}°C"; break
                    ...
                    if os.path.exists(vused) and os.path.exists(vtotal):
                        ...
                        vram = f"{u} / {t} MB"
                    return {"name": "AMD GPU", "load": load, "vram": vram, "temp": temp}
            return None
        except: return None
    ...
    def update_telemetry(self):
        ...
        if "NVIDIA" in hw:
            ...
            if amd:
                ...
                self.lbl_gpu_temp.setText(f"Temp: {amd['temp']}"); self.lbl_gpu_load.setText(f"Carga: {amd['load']}")
        else:
            self.lbl_gpu_name.setText("Modo CPU"); self.lbl_gpu_vram.setText("-"); self.lbl_gpu_temp.setText("-"); self.lbl_gpu_load.setText("-")
