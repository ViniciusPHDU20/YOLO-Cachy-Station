...
PY_BINARY = "python3.11" if shutil.which("python3.11") else sys.executable
...
def bootstrap():
    ...
    if missing:
        ...
        subprocess.run([sys.executable, "-m", "pip", "install", *missing, "pyyaml", "nvidia-ml-py3", "--break-system-packages"], capture_output=True)
...
class MainWindow(QMainWindow):
        ...
        try:
            try: import pynvml
                ...
                if os.path.exists(lib_root):
                    for py_dir in os.listdir(lib_root):
                        vlib = os.path.join(lib_root, py_dir, "site-packages")
                        if os.path.exists(vlib) and vlib not in sys.path: 
    ...
    def init_ui(self):
        ...
        header.addStretch()
        header.addWidget(QLabel(f"ARCH: {platform.machine()} | KERNEL: {platform.release()}"))
    ...
    def setup_train_tab(self):
        ...
        self.combo_gpu = QComboBox(); self.combo_gpu.addItems(["NVIDIA CUDA", "AMD ROCm", "CPU Only"])
    ...
    def get_amd_gpu_data(self):
        try:
            for card in [d for d in os.listdir("/sys/class/drm") if d.startswith("card")]:
                base = f"/sys/class/drm/{card}/device"
                target_file = f"{base}/gpu_busy_percent"
                if os.path.exists(target_file):
                    with open(target_file, "r") as f: load = f"{f.read().strip()}%"
                    temp = "0°C"
                    hwmon = f"{base}/hwmon"
                    if os.path.exists(hwmon):
                        for hdir in os.listdir(hwmon):
                            for i in range(1, 4):
                                tf = f"{hwmon}/{hdir}/temp{i}_input"
                                if os.path.exists(tf):
                                    with open(tf, "r") as f: 
                                        val = f.read().strip()
                                        if val: temp = f"{int(val) // 1000}°C"; break
                            if temp != "0°C": break
                    
                    vused = f"{base}/mem_info_vram_used"; vtotal = f"{base}/mem_info_vram_total"
                    vram = "- / - MB"
                    ...
                    if os.path.exists(f"{base}/device"):
                        with open(f"{base}/device", "r") as f: name = f"AMD RADEON ({f.read().strip()})"
                        
                    return {"name": name, "load": load, "vram": vram, "temp": temp}
            return None
        except: return None
    ...
    def update_telemetry(self):
        ...
        if "NVIDIA" in hw:
            ...
            if amd:
                self.lbl_gpu_name.setText(f"GPU: {amd['name']}"); self.lbl_gpu_vram.setText(f"VRAM: {amd['vram']}")
                self.lbl_gpu_temp.setText(f"TEMP: {amd['temp']}"); self.lbl_gpu_load.setText(f"LOAD: {amd['load']}")
        else:
            self.lbl_gpu_name.setText("GPU: CPU_MODE"); self.lbl_gpu_vram.setText("-"); self.lbl_gpu_temp.setText("-"); self.lbl_gpu_load.setText("-")
        ...
        def task():
            try:
                ...
                if "NVIDIA" in gpu:
                    ...
                    subprocess.run([PIP_BIN, "install", "numpy==1.26.4", "torch", "torchvision", "--index-url", "https://download.pytorch.org/whl/rocm6.0"], env=env)
    ...
    def start_training(self):
        ...
        gpu = self.combo_gpu.currentText(); device = "0" if ("NVIDIA" in gpu or "AMD" in gpu) else "cpu"
...
torch.load = new_load
model = YOLO('{model}')
model.train(data='{yaml_f}', epochs={p['e']}, imgsz={p['i']}, batch={p['b']}, device='{device}', workers=0, amp=False, project='{os.path.join(self.workspace, RUNS_DIR)}', name='train', exist_ok=True, resume={is_res})
"""
